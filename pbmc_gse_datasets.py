...
# Calculate the mean detected-gene count per cell without converting the sparse matrix to dense form.
def average_genes_per_cell(adata):
    """Count the average number of nonzero genes per cell."""
    # Return zero for an empty AnnData rather than computing a mean over no cells.
    if not adata.n_obs:
        # Return zero for an empty AnnData so QC logging stays defined without NumPy empty-mean warnings.
        return 0.0
    # Use sparse `getnnz` when possible; dense arrays require a separate nonzero-count implementation.
    if sparse.issparse(adata.X):
        # For sparse counts, use nonzero counts per CSR row directly; this measures detected genes without allocating a dense boolean matrix.
        return float(adata.X.getnnz(axis=1).mean())
    # Use `count_nonzero` only for an already-dense matrix; this is the dense fallback to the sparse `getnnz` path.
    return float(np.count_nonzero(adata.X, axis=1).mean())


# Combine technical libraries from one cohort while keeping the union of genes and globally unique cell identifiers.
def _concat(objects: list[ad.AnnData]) -> ad.AnnData:
    ...
    for i, obj in enumerate(objects):
        ...
        objects[i] = standardize_genes(obj, report=False)
    # Drop the loop-local object reference after replacing the list element, helping large temporary AnnData objects become collectible sooner.
    del obj
...
def build_gse(
    ...
    if accession == "GSE341191":
        ...
        # Collect parsed `GSE341191` libraries temporarily; raw inputs can be deleted after parsing and concatenated afterward.
        objects = []
...
def _raw_subset(source, keep, donors, draws, library) -> ad.AnnData:
    """Read only selected raw RNA rows and retain source cell/donor/draw IDs."""
    # Convert the cell mask to explicit row positions for reliable backed/raw matrix slicing.
